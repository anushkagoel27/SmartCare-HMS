...
def appointment_book(request):
    ...
    if request.method == 'POST':
        ...
        if name and phone and doctor_id:
            ...
            try:
                ...
                messages.success(
                    request,
                    f'Appointment booked with {doctor.name} on {appointment_obj.appointment_date} at {appointment_obj.appointment_time}'
                )
                return redirect('patient-dashboard')
    ...
    return redirect('appointment')  # GET request → go back to form
